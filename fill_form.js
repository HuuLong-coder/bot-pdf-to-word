...
const SCORE_URL =
  'https://docs.google.com/forms/d/e/1FAIpQLSdPPnpMVvUAIsajqJHpwk-GHpKkgEEreGs49GcHBsxKAmyAtg/viewscore?viewscore=AE0zAgDoxdbDIfdFIRLgWEEIuwWIL7ypnYwNLeXWQd0AW9nAUZ0J_68Y3Gb8bOI9yduspQQ&fbclid=IwY2xjawQ7HBlleHRuA2FlbQIxMABicmlkETFwMjUxZEZHTkFBRWRJZnZ3c3J0YwZhcHBfaWQQMjIyMDM5MTc4ODIwMDg5MgABHmwqOumLGxuhZDxZIdt1hBZqJ_ZsdrHeGfk0vlD8gEC7Mqlq_idl2Ujco3_d_aem_1vgAxWGmmvtHPLov_pQAcQ';
const FORM_URL =
  'https://docs.google.com/forms/d/e/1FAIpQLSdPPnpMVvUAIsajqJHpwk-GHpKkgEEreGs49GcHBsxKAmyAtg/viewform';
...
async function tryOpenChromeProfileContext() {
  console.log(`Đang mở Chrome profile: ${CHROME_PROFILE}`);
  console.log('⚠  Nếu Chrome đang mở nền, Playwright có thể không mở được profile này.\n');
  ...
  try {
    ...
    return { context, page, mode: 'profile' };
  } catch (error) {
    const msg = String(error && error.message ? error.message : error);
    console.log('⚠ Không mở được profile Chrome trực tiếp.');
    console.log(`   Chi tiết: ${msg.split('\n')[0]}`);
    console.log('   Chuyển sang chế độ session lưu tại auth.json...\n');
    return null;
  }
}

async function exportAuthFromChromeProfile(authFilePath, profileDir) {
  console.log(`Đang tạo ${path.basename(authFilePath)} từ Chrome profile: ${profileDir}`);
  console.log('⚠ Hãy đóng tất cả cửa sổ Chrome trước khi tiếp tục.');

  try {
    execSync('taskkill /F /IM chrome.exe /T', { stdio: 'ignore' });
  } catch {
    // Chrome có thể đã tắt sẵn
  }

  const srcProfilePath = path.join(CHROME_USER_DATA, profileDir);
  if (!fs.existsSync(srcProfilePath)) {
    console.log(`⚠ Không tìm thấy profile: ${srcProfilePath}`);
    return false;
  }

  // Chrome mới chặn remote-debugging với default user-data-dir.
  // Tạo user-data-dir tạm (non-default) rồi copy profile vào để Playwright mở hợp lệ.
  const tempUserDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pw-auth2-'));
  const tempDefaultProfile = path.join(tempUserDataDir, 'Default');
  fs.mkdirSync(tempDefaultProfile, { recursive: true });
  ...
  // Copy toàn bộ profile nguồn vào Default của user-data-dir tạm.
  fs.cpSync(srcProfilePath, tempDefaultProfile, { recursive: true, force: true });

  let ctx;
  try {
    ctx = await chromium.launchPersistentContext(tempUserDataDir, {
      channel: 'chrome',
      headless: false,
      slowMo: 50,
    });
  ...
  if (!signedIn || !isSignedInPageTitle(await page.title())) {
    console.log('⚠ Profile này chưa đăng nhập Google hoặc bị khóa.');
    console.log('  Hãy đăng nhập sẵn trên Chrome profile này rồi chạy lại.');
    await ctx.close();
    try { fs.rmSync(tempUserDataDir, { recursive: true, force: true }); } catch {}
    return false;
  }

  await ctx.storageState({ path: authFilePath });
  await ctx.close();
  try { fs.rmSync(tempUserDataDir, { recursive: true, force: true }); } catch {}
  console.log(`✓ Đã tạo ${path.basename(authFilePath)} từ profile ${profileDir}.\n`);
  return true;
}

async function openSessionContextWithAuthFile(authFilePath, modeLabel, options = {}) {
  const personalMode = !!options.personalMode;
  const personalProfile = options.personalProfile || CHROME_PERSONAL_PROFILE_DEFAULT;

  if (personalMode && !fs.existsSync(authFilePath)) {
    const ok = await exportAuthFromChromeProfile(authFilePath, personalProfile);
    if (!ok) {
      throw new Error('Không tạo được auth2 từ profile Chrome cá nhân.');
    }
  }
  ...
  if (!signedIn || !isSignedInPageTitle(await page.title())) {
    if (personalMode) {
      console.log('⚠ Session cá nhân hết hạn. Đang refresh từ Chrome profile cá nhân...');
      await context.close();
      await browser.close();
      const ok = await exportAuthFromChromeProfile(authFilePath, personalProfile);
      if (!ok) {
        throw new Error('Refresh auth2 thất bại.');
      }
      const browser2 = await chromium.launch({ headless: false, slowMo: 50, channel: 'chrome' });
      const context2 = await browser2.newContext({ storageState: authFilePath });
      const page2 = await context2.newPage();
      console.log(`✓ Đăng nhập sẵn bằng session ${path.basename(authFilePath)} (${modeLabel}).\n`);
      return { context: context2, page: page2, mode: 'session', browser: browser2 };
    }

    await page.goto('https://accounts.google.com', { waitUntil: 'networkidle' });
    await waitForEnter(
      '\n[BƯỚC ĐĂNG NHẬP] Hãy đăng nhập Google trên cửa sổ vừa mở.\n' +
...
(async () => {
  console.log('=== Google Form Auto-Fill (multi-page) ===\n');

  const modeRaw = await promptInput(
    'Chọn chế độ chạy:\n' +
    '  1) Bình thường (dùng auth.json)\n' +
    '  2) Email cá nhân (dùng auth2.json)\n> '
  );
  const mode = normalize(modeRaw);
  const personalMode = mode === '2' || mode.includes('cá nhân') || mode.includes('ca nhan') || mode.includes('personal');

  const selectedAuthFile = personalMode ? AUTH_FILE_PERSONAL : AUTH_FILE;
  const selectedModeLabel = personalMode ? 'email cá nhân' : 'bình thường';

  let personalProfile = CHROME_PERSONAL_PROFILE_DEFAULT;
  if (personalMode) {
    personalProfile = await chooseChromeProfileInteractive(CHROME_PERSONAL_PROFILE_DEFAULT);
  }

  let personalEmail = PERSONAL_EMAIL_FROM_ARGS;
  if (personalMode && !personalEmail) {
  ...
  console.log(`Session file: ${path.basename(selectedAuthFile)}`);
  if (personalEmail) {
